...
def run_with_timeout(
    code: str, timeout_seconds: int = 20
...
def evaluate(path: Path) -> Optional[dict[str, Any]]:
    """
    ...
    try:
        ...
        assert isinstance(centers, np.ndarray), "Centers was not of type np.ndarray"
        assert isinstance(radii, np.ndarray), "Radii was not of type np.ndarray"
        ...
        return {
            "fitness": float(sum_radii) if valid else 0.0,
